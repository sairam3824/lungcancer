#!/usr/bin/env python3
"""
Lung Cancer Classification - CSPDarkNet101
...
# Model Architecture: CSPDarkNet101
...
class CSPDarkNet101(nn.Module):
    def __init__(self, num_classes=3):
        super(CSPDarkNet101, self).__init__()
        self.conv1 = ConvBlock(3, 32, 3)
        self.stage1 = CSPBlock(32, 64, num_blocks=2)
        self.stage2 = CSPBlock(64, 128, num_blocks=4)
        self.stage3 = CSPBlock(128, 256, num_blocks=23)  # Much deeper
        self.stage4 = CSPBlock(256, 512, num_blocks=23)  # Much deeper
        self.stage5 = CSPBlock(512, 1024, num_blocks=5)
...
def plot_confusion_matrix(y_true, y_pred, class_names, save_path):
    ...
    plt.title('Confusion Matrix - CSPDarkNet101')
...
if __name__ == "__main__":
    print("=" * 70)
    print(" Lung Cancer Classification - CSPDarkNet101")
    print("=" * 70)
    ...
    os.makedirs('cspdarknet101_results', exist_ok=True)
    
    print("\n📂 Loading dataset...")
    train_loader, val_loader, class_names, eval_transform = load_data('dataset', batch_size=8)
    
    num_classes = len(class_names)
    model = CSPDarkNet101(num_classes=num_classes).to(device)
    total_params = sum(p.numel() for p in model.parameters())
    print(f"\n🔧 CSPDarkNet101 initialized with {total_params:,} parameters")
    
    print("\n🚀 Starting training...")
    model, history, val_labels, val_preds = train_model(model, train_loader, val_loader, class_names, num_epochs=10)
    
    print("\n💾 Best model saved as 'cspdarknet101_best.pth'")
    
    plot_training_history(history, 'cspdarknet101_results/training_history.png')
    plot_confusion_matrix(val_labels, val_preds, class_names, 'cspdarknet101_results/confusion_matrix.png')
    
    final_metrics = calculate_metrics(val_labels, val_preds, class_names)
    
    with open('cspdarknet101_results/metrics.json', 'w') as f:
        json.dump(final_metrics, f, indent=4)
    print("\n📊 Metrics saved to 'cspdarknet101_results/metrics.json'")
    
    report = classification_report(val_labels, val_preds, target_names=class_names)
    with open('cspdarknet101_results/classification_report.txt', 'w') as f:
        f.write("CSPDarkNet101 Classification Report\n")
        f.write("=" * 70 + "\n\n")
        f.write(report)
    ...
    if os.path.exists('test images'):
        ...
        with open('cspdarknet101_results/test_results.json', 'w') as f:
